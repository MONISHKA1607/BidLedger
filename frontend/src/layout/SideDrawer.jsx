  ...
  const getNavClass = (to, aliases) =>
    `${navLinkBase} ${
      isActiveRoute(to, aliases)
        ? "bg-brass/10 text-brass-strong ring-1 ring-brass/20"
        : "text-stone-600 hover:bg-stone-100 hover:text-brass-strong"
    }`;
  const getRoleClass = (to, aliases) =>
    `${roleLinkBase} ${
      isActiveRoute(to, aliases)
        ? "bg-ink text-white shadow-sm hover:bg-brass-strong"
    ...
    <>
      ...
      {show && (
        <button
          ...
          className="fixed inset-0 z-30 bg-ink/50 backdrop-blur-sm xl:hidden"
        />
      )}
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-stone-200 pb-4">
          ...
          <button
            ...
            onClick={() => setShow(false)}
            aria-label="Close navigation"
            className="inline-flex h-10 w-10 items-center justify-center rounded-md text-stone-500 transition hover:bg-stone-100 hover:text-ink xl:hidden"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <nav aria-label="Primary navigation" className="mt-5 flex flex-1 flex-col gap-5">
          <div className="grid gap-1.5">
            <p className="px-3 text-xs font-bold uppercase text-stone-500 tracking-[0.12em]">
              Marketplace
            </p>
          ...
          {isAuthenticated ? (
            <>
              ...
              {showWalletCard && (
                <div className="rounded-lg border border-brass/20 bg-brass/10 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-bold uppercase text-brass-strong tracking-[0.12em]">
                      Wallet
                    </p>
                    <BiWallet className="text-xl text-brass-strong" />
                  </div>
                  <p className="font-mono-brand mt-2 text-2xl font-semibold text-ink">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {canDeposit && (
                      <Link
                        to="/wallet#deposit"
                        className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-md bg-ink px-2 py-2 text-sm font-semibold text-white transition hover:bg-brass-strong"
                      >
                    ...
                    {canWithdraw && (
                      <Link
                        to="/wallet#withdraw"
                        className={`inline-flex min-h-10 items-center justify-center gap-1.5 rounded-md border border-brass/30 bg-white px-2 py-2 text-sm font-semibold text-brass-strong transition hover:bg-brass/10 ${
              ...
              <div className="grid gap-1.5">
                ...
                <div className="mb-1 flex min-w-0 items-center gap-2 rounded-md bg-stone-50 px-3 py-2">
                  <BiUser className="shrink-0 text-xl text-brass-strong" />
                ...
                {isDemo ? (
                  <button
                    ...
                    className={`${navLinkBase} text-left text-stone-600 hover:bg-stone-100 hover:text-brass-strong`}
                  >
                    <BiLogOut /> Exit demo
                  </button>
                ) : (
        ...
        <div className="mt-5 rounded-lg border border-stone-200 bg-stone-50 p-3">
          <p className="flex items-center gap-2 text-xs font-bold uppercase text-stone-500 tracking-[0.12em]">
