      ...
      {showInitialSpinner ? (
        ...
        <article className="app-page">
          <section className="app-container">
            ...
            <div className="mb-6 grid gap-3 rounded-lg border border-stone-200 bg-white p-4 shadow-sm shadow-ink/[0.03] xl:grid-cols-[1fr_auto_auto_auto]">
              <label className="flex min-w-0 items-center gap-3 rounded-md border border-stone-300 px-3 py-2 focus-within:border-brass focus-within:ring-2 focus-within:ring-brass/15">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 md:col-span-2 xl:col-span-4">
                  ...
                  {MARKETPLACE_STATUS_OPTIONS.map((status) => (
                    <button
                      ...
                      className={`rounded-full px-3 py-1.5 text-sm font-semibold transition ${
                        query.status === status
                          ? "bg-ink text-white"
                          : "bg-stone-100 text-stone-600 hover:bg-brass/10 hover:text-brass-strong"
                      }`}
                    >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className="rounded-md bg-stone-100 px-3 py-2 text-sm font-semibold text-stone-600">
                    {pageStart}-{pageEnd} of {totalItems} shown
                  </span>
                  <button
                    type="button"
                    onClick={refreshAuctions}
                    className="inline-flex items-center gap-2 rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold text-stone-600 transition hover:border-brass/40 hover:text-brass-strong"
                  >
                  ...
                  {hasActiveFilters && (
                    <button
                      ...
                      className="rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold text-stone-600 transition hover:border-brass/40 hover:text-brass-strong"
                    >
                      Reset
                    </button>
                  )}
            ...
            {auctionListError ? (
              ...
              <div className="flex min-h-[260px] flex-col items-center justify-center rounded-lg border border-dashed border-stone-300 bg-white p-8 text-center shadow-sm">
                <SlidersHorizontal className="mb-3 h-7 w-7 text-brass-strong" />
                ...
                <button
                  type="button"
                  onClick={resetFilters}
                  className="mt-4 rounded-md border border-stone-300 px-4 py-2 font-semibold text-stone-600 transition hover:border-brass/40 hover:text-brass-strong"
                >
                  Reset filters
                </button>
              </div>
...
const FilterSelect = ({ label, value, onChange, options }) => (
  <label className="grid min-w-[170px] gap-1">
    ...
    <select
      ...
      className="rounded-md border border-stone-300 bg-white px-3 py-2 text-ink outline-none transition focus:border-brass focus:ring-2 focus:ring-brass/15"
    >
      {options.map((option) => {
...
const PaginationControls = ({
  ...
  <nav
    className="mt-8 flex flex-col items-center justify-between gap-3 rounded-lg border border-stone-200 bg-white p-4 shadow-sm sm:flex-row"
  ...
  >
    ...
    <div className="flex items-center gap-2">
      <button
        ...
        className="inline-flex min-h-10 items-center gap-2 rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold text-stone-600 transition hover:border-brass/40 hover:text-brass-strong disabled:cursor-not-allowed disabled:opacity-50"
      >
...
const TrustItem = ({ icon: Icon, text }) => (
  <div className="trust-item">
    <Icon className="h-5 w-5 shrink-0 text-brass-strong" />
    <span>{text}</span>
  </div>
);
